    ...
    if request.method == "POST":
        ...
        if p_item_object:
            if current_user.can_purchase(p_item_object):
                ...
                flash(f"Oops!! You don't have enough money to purchase {p_item_object.name}",category='danger')
        ...
        if s_item_object:
            if current_user.can_sell(s_item_object):
                ...
                flash(f"Something went wrong!! Can't sell {s_item_object.name}", category='danger')
...
def register_page():
    ...
    if form.errors != {}:
        for err_msg in form.errors.values():
            flash(f'There is an error in creating a user: {err_msg}',category='danger')
...
def logout_page():
    logout_user()
    flash("Your session has been logged out!",category='info')
